...
Atheneum is extensively tested on multiple operating systems using
...
Atheneum should also work on most other Unix-like systems but is not
...
[
  {
    ...
    winner: {
        address: addr,    (string) AEM MN Address,
...
[
  {
    ...
    "addr": addr,      (string) Masternode AEM address
...
Atheneum now enables bloom filters by default to support SPV clients like mobile wallets. This feature can be disabled by using the `-peerbloomfilters` option on startup.
...
- #195 `aee05fe` [ZMQ] ZMQ integration for AEM (Mrs-X)
...
### Tests and QA
- #191 `3a778c3` [Tests] Fix the unit test suite for use with AEM (Fuzzbawls)
